...
class FormDropDown extends React.Component {
  ...
  updateCategoryDetails = event => {
    ...
    if (CategoryName === "" || CategoryDescription === "" || FormType === "") {
      ...
      fetch("http://localhost:4000/api/v1/category/" + CategoryID, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  loadPetDetails(PetOwnerID) {
    ...
    fetch(
      'http://localhost:4000/api/v1/pet/?where={"PetOwnerID":}' + PetOwnerID
    )
      .then(response => response.json())
      .then(categoryList => {
  ...
  DeleteCategory(ServiceID) {
    fetch("http://localhost:4000/api/v1/category/" + ServiceID, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json"
      }
