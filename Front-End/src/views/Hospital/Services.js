...
class servicesCmp extends React.Component {
  ...
  componentDidMount() {
    fetch('http://localhost:4000/api/v1/service?HID='+localStorage.getItem('UserTypeID'))
    .then((response) => response.json())
    .then(serviceList => {
        this.setState({ PostsListFour: serviceList });
    });
  }
  // fetch(`http://localhost:4000/api/v1/service?where=${JSON.stringify({ HID: '644662ef42bb21884c60c6ba' })}`)
  // .then(response => response.json())
  // .then(data => console.log(data))
  // .catch(error => console.error(error));
  DeleteService(ServiceID){

    fetch('http://localhost:4000/api/v1/service/' + ServiceID, {
      method: 'DELETE',
      headers: {
